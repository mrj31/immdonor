...
Immdonor is a [GraphQL API](https://graphql.org/) of [Immport](https://www.immport.org/) metadata hosted on [Hasura Cloud](https://hasura.io/). This API lets you request exactly what study information you need from the [Immport data model](https://www.immport.org/shared/dataModel) and returns a JSON output which can be converted to a R dataframe!

🚩This is a fast and easy way to link [Flow Cytometry Standard (.FCS)](https://en.wikipedia.org/wiki/Flow_Cytometry_Standard) files to their metadata for analysis with [Cytoverse](https://cytoverse.org/). You will need to have the FCS files saved locally (see quick_fetch.R)
...
knitr::kables(
  list(
    ...
    knitr::kable(mucosal, col.names = c('Mucosal Organs', 'Count'), valign = 't')
    ))
